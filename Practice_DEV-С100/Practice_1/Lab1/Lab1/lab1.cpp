...
int main()
{
	...
	stop		// эта подстановка нужна, только для того, чтобы поставить точку останова и посмотреть значения переменных
...
// *******************************************************
	...
	for (int i = 0; i < 3; i++)
	{
		...
		int nLocal1 = 100;
		int nLocal2 = 200;
		static int nStatic2 = (nLocal2 += 1) * 2;   // сначала вычисление выражения, потом присвоение // static инициализируется 1 раз в блоке, дальше используется
		nStatic1 += 1;
		nStatic2 += 1;
		nLocal1 += 1;
		stop
...
// *******************************************************
...

	{
		...
		//ошибки при компиляции 
			...
			const int nN = 0;		//  константа требует явную инициализацию переменной
