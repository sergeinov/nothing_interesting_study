...
// ********************************************************
	...
	#define calculateCircle(radius)	(M_PI*(radius)*2)
	...
	// протестируйте написанный макрос для следующих ситуаций:
	{
		...
		//#define PI 3.14
		#define LENGHT(radius)  (2.0 * M_PI * (radius))		// первое преобразование для второго теста. Второе преобр. для первого теста
...
// *******************************************************
	...
	#define	EXCHANGE(x,y) {int tmp = x; x = y; y = tmp;}		// избавляемся от переопределения temp фигурными скобками
...
// *******************************************************
	...
	//Интерпретируте наблюдаемые результаты
	{
		...
		pd2+=2;							//pd2 указывает на адрес за границей массива // 0
		...
		int nNumber=pd2 - pd1;			//   nNumber= 1	(8 байт)   pd1= 0x010ffac8 (3.0)   pd2= 0x010ffad0  (2.0)
		// результат означает, на сколько элементов "double" один адрес отстает от другого. 
		//Разница 8 байт типа double(Т.к. у казатели pd1/pd 2 - double)
...
// *******************************************************
	...
	// отбрасывает старшую часть, оставляя младшую. А младшая у нас мантисса
