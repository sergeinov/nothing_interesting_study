...
//2)дальше, воспользовавшись этими указателями, переставляем местами элементы i-того и i+1-ого слоев
{
	...
	double dArray[N][M][K] = {};
	double(*pdArray)[M][K] = &dArray[0];	// эквивалентный указатель  для имени двухмерного массива 
	double** ptrPdArray = reinterpret_cast<double**>(&pdArray);  // указатель на  массив указателей
	...
	for (size_t i = 0; i < N - 1; i++)
	{
		if (**ptrPdArray < (**ptrPdArray +1))
		{
			...
			std::cout << *ptrPdArray << std::endl;
			std::cout << (*ptrPdArray + 1)<< std::endl;
